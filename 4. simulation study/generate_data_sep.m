function data=generate_data_sep(trace, cov_path, n_sub, n_pairs, seq_post, set_rng, ind_sel, phi, returnee)    
% GENERATE_DATA_SEP generates the data with separated components
% Inputs:
...
%       contains the generated data.
    ...
    % mean component
    alpha = mean(trace.alpha(:, seq_post), 2) + [2; zeros(dim_cov_pop-1, 1)];
    ...
    delta = mean(trace.delta(:, seq_post), 2)+[1; zeros(dim_cov_pop-1, 1)];
